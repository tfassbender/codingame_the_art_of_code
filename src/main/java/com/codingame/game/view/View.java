...
public class View {
	...
	public View(GraphicEntityModule graphicEntityModule, GameMap map, Map<Field, Vector2D> initialPositions) {
		...
		gunner_left_right_blue = graphicEntityModule.createSpriteSheetSplitter().setName("Gunner_Blue").setOrigCol(0)
				.setOrigRow(0).setImageCount(6).setImagesPerRow(6).setHeight(48).setWidth(48)
				.setSourceImage("Gunner_Blue_Run.png").split();
		gunner_left_right_red = graphicEntityModule.createSpriteSheetSplitter().setName("Gunner_Red").setOrigCol(0)
				.setOrigRow(0).setImageCount(6).setImagesPerRow(6).setHeight(48).setWidth(48)
	...
	public void drawPlayerInfos(Player player1, Player player2) {
		...
		graphicEntityModule.createText(player1.getNicknameToken()).setFontFamily("courier")
				.setFontWeight(FontWeight.BOLD).setFontSize(40) //
				.setX(240).setY(80).setAnchor(0.5);

		graphicEntityModule.createText("Fields:\nTroops:\nSupply:").setFontFamily("courier")
		...
		graphicEntityModule.createText(player2.getNicknameToken()).setFontFamily("courier")
				.setFontWeight(FontWeight.BOLD).setFontSize(40) //
	...
	public void drawLegend(Set<Region> regions) {
		graphicEntityModule.createText("Regions").setFontFamily("courier").setFontWeight(FontWeight.BOLD)
				.setFontSize(40) //
		...
		String regionNumFields = regions.stream().map(region -> Integer.toString(region.fields.size()) + "F -> ")
				.collect(Collectors.joining("\n"));
		String regionBonuses = regions.stream().map(region -> Integer.toString(region.getBonusTroops()) + "T")
				.collect(Collectors.joining("\n"));
	...
	public void drawConnections(Set<Pair<Field, Field>> connections, Set<Region> regions) {
		Set<Field> fields = connections.stream().flatMap(pair -> Stream.of(pair.getKey(), pair.getValue()))
		...
		for (Pair<Field, Field> connection : connections) {
			...
			ConnectionFinder connectionFinder = new ConnectionFinder(GAME_FIELD_WIDTH, GAME_FIELD_HEIGHT, GAME_FIELD_X,
					GAME_FIELD_Y, pos1, pos2);
	...
	private void createMovementAnimationEntity(Pair<Field, Field> connection, Vector2D pos1, Vector2D pos2,
			boolean faceRight) {
		// troops animations
		SpriteAnimation animationP1 = graphicEntityModule.createSpriteAnimation().setImages(gunner_left_right_red)
				.setX((int) pos1.x).setY((int) pos1.y).setLoop(true).setAnchor(0.5).setScaleX(faceRight ? 1 : -1);
		SpriteAnimation animationP2 = graphicEntityModule.createSpriteAnimation().setImages(gunner_left_right_blue)
				.setX((int) pos1.x).setY((int) pos1.y).setLoop(true).setAnchor(0.5).setScaleX(faceRight ? 1 : -1);
	...
	public void updatePlayerStats(Owner player, int fields, int troops, int deployable) {
		...
		if (player == Owner.PLAYER_1) {
			...
			throw new IllegalArgumentException(
					"The parameter 'player' should be PLAYER_1 or PLAYER_2 but was: " + player);
		}
	}
	...
	public void animatePicks(Set<Field> fields, Set<PickEvent> picksPerformed) {
		...
		if (pickGraphics.size() == 0) {
			pickGraphics.put(Owner.PLAYER_1,
					graphicEntityModule.createSprite().setImage("pointing_finger.png")
							.setTint(getColorForOwner().get(Owner.PLAYER_1)).setAlpha(1).setAnchor(0.5)
							.setBaseWidth(100).setBaseHeight(100));
			pickGraphics.put(Owner.PLAYER_2,
					graphicEntityModule.createSprite().setImage("pointing_finger.png")
							.setTint(getColorForOwner().get(Owner.PLAYER_2)).setAlpha(1).setAnchor(0.5)
							.setBaseWidth(100).setBaseHeight(100));
		}
		...
		for (PickEvent pick : picksPerformed) {
			...
			hand.setX((int) (position.x + xOffset), Curve.NONE).setY((int) position.y - 100, Curve.NONE)
	...
	public void animateDeployments(List<Action> actions1, List<Action> actions2) {
		...
		for (Field field : deployText.keySet()) {
			...
			int deployedTroops = actions.stream()
					.filter(a -> a.getTargetId() == field.id && a.getType() == Action.Type.DEPLOY)
	...
	public void animateMovements(MovementEvents events, Set<Field> fields) {
		...
		for (Pair<Field, Field> key : keys) {
			...
			SpriteAnimation troops = steps.get(0).owner == Owner.PLAYER_1 ? spriteSelection.getKey()
					: spriteSelection.getValue();
			...
			for (MovementStep step : steps) {
				...
				case FIGHT:
					...
					bullet.setX((int) shootFrom.x).setY((int) shootFrom.y).setRadius(5)
							.setFillColor(0xFFF000);
					...
					if (!route.isTrivialRoute() || !troopNavi.getRoute(step.fightWithMovement).isTrivialRoute()) {
						estimatedConnection = new ConnectionFinder(GAME_FIELD_WIDTH, GAME_FIELD_HEIGHT,
								GAME_FIELD_X, GAME_FIELD_Y, shootFrom, shootAt);
					...
					if (makeDirectBullet) {
						...
						Route bulletTrajectory = new Route(shootFrom, estimatedConnection.getIntersection1(),
								estimatedConnection.getIntersection2(), shootAt);

						List<Pair<Double, Vector2D>> bulletEvents = bulletTrajectory
								.getTravelWithRelativeTimestamps(0, 1);
				...
				case FORWARD:
					if (lastStep) { // move to the end position (from starting or fighting position)
						timedPositioning = route.getTravelWithRelativeTimestamps(steps.size() <= 1 ? 0 : relativeFightPosition,
								1);
	...
	public void resetAnimations(TurnType turnType) {
		...
		if (turnType != TurnType.MOVE_TROOPS && cachedFieldPositions != null) {
			for (Pair<Field, Field> key : moveAnimations.keySet()) {
				...
				for (SpriteAnimation troop : new SpriteAnimation[] { spritesRedBlue.getKey(),
						spritesRedBlue.getValue() }) {
					troop.setAlpha(0, Curve.NONE).setX((int) startPosition.x, Curve.NONE).setY((int) startPosition.y,
							Curve.NONE);
				}

				Text troopText = moveText.get(key);
				troopText.setX((int) startPosition.x, Curve.NONE)
						.setY((int) startPosition.y + TEXT_TROOPS_Y_OFFSET, Curve.NONE).setAlpha(1, Curve.NONE)
						.setText("");
			}
		}
